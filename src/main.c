...
static void draw_background_oneline(uint8_t lcdc, int y, Uint32 buf[]) {
...
static void draw_window_oneline(uint8_t lcdc, int y, Uint32 buf[]) {
...
static void draw_sprite_oneline(uint8_t lcdc, int scr_y, Uint32 buf[]) {
	...
	while(!quit){
		...
		if(INTERNAL_IO[IO_LCDC_R]&0x80){
			...
			while(INTERNAL_IO[IO_LY_R]<=143){
				...
				uint8_t ly = INTERNAL_IO[IO_LY_R];
				change_lcdmode(2); cpu_exec(83);
				change_lcdmode(3); cpu_exec(175);

				if(lcdc&0x1)
					draw_background_oneline(lcdc, ly, bitmap);
				else
					for(int i=0; i<1024; i++)
						bitmap[i] = ABSCOLOR[0];

				if(lcdc&0x20)
					draw_window_oneline(lcdc, ly, bitmap);
				if(lcdc&0x2)
					draw_sprite_oneline(lcdc, ly, bitmap);
